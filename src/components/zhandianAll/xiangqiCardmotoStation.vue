      ...
      <el-dialog :title="name" :visible.sync="stations" width="30%">
        <el-form>
          ...
          <el-form-item label="类型:">
            <el-select v-model="value" placeholder="时间" disabled>
              <el-option
...
export default {
  ...
  methods: {
    search() {
      ...
      let url = `admin/api/packages?token=${toKen}&page=${this.newpark}&row=${this.pagesizex}&agent=0&station=${this.stationsId}&charger=0&agent=0&service=10&source=rfid&keyword=${this.msgss}`;
    ...
    add() {
      ...
      if (this.lest_id) {
        let url = `/admin/api/package/${this.lest_id}?token=${this.token}&name=${this.a}&pay=${this.b}&duration=${this.c}&energy=${this.energyC}&amount=${this.amountC}&type=${this.value}`;
        console.log("putUrl");
        console.log(url);
        ...
        let url =
          ...
          "&type=" +
          this.value +
          "&agent=0&charger=0&source=rfid&service=10";
        console.log("Purl");
        console.log(url);
    ...
    getParksMes() {
      let toKen = this.token.replace(/\"/g, "");
      let url = `admin/api/packages?token=${toKen}&page=${this.newpark}&row=${this.pagesizex}&agent=0&station=${this.stationsId}&charger=0&agent=0&service=10&source=rfid`;
      console.log("getUrl");
      console.log(url);
      this.$axios.get(url).then(res => {
        if (res.status == 200) {
