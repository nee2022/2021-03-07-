								...
								<template slot-scope="scope">
									<div class="leftBox" :class="{ BGactive:scope.row.id == isActive }" @click="changeBg(scope.row.id)">
										...
										<div class="leftBox-con" @click="getParksMes(scope.row.id),(scope.row.station)">
											...
											<div class="parkIcon">
												<div>
													<span class="parkIcon-word">电单车</span>
												</div>
												<div>
				...
				<div class="rightbx">
					<template>
						<el-table :data="parkList" stripe style="width:100%">
							...
							<el-table-column show-overflow-tooltip prop="dev_id" label="设备编号">
							</el-table-column>
							...
							<el-table-column show-overflow-tooltip prop="contact_name" label="联系人">
							</el-table-column>
							<el-table-column show-overflow-tooltip prop="contact_phone" label="联系电话">
							</el-table-column>
	...
	export default {
		...
		methods: {
			...
			getRoadMes() {
				...
				// console.log(toKen)
				this.$axios.get("admin/api/stations/4?token=" + toKen + "&page=" + this.pagenum + "&row=6")
					.then(res => {
						if (res.status == 200) {
							const {
								stations: result
				...
				this.$refs.addFormRef.validate(valid => {
					if (!valid) {
						...
						this.$axios.post("/admin/api/charger?token=" + this.addForm.token + "&dev_id=" + this.addForm.dev_id + "&name=" +
								this.addForm.name + "&latitude=" + this.addForm.latitude +  "&contact_phone=" + this.addForm.contact_phone + "&contact_name=" + this.addForm.contact_name + "&longitude=" + this.addForm.longitude + "&gun=" + this.addForm.gun + "&mac=" + this.addForm.mac + "&type=7&station=" + this.getFristID)
							.then(res => {
								if (res.status !== 200) {
